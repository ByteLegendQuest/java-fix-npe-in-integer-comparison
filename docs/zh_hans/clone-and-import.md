...
`git clone https://git.bytelegend.com/ByteLegendQuest/java-fix-npe-in-integer-comparison`

这会在本地创建一个名为`java-fix-npe-in-integer-comparison`的项目文件夹，你可以把它移动到你喜欢的任何地方去。

打开`IDEA`，选择`File` - `Open`，选中刚刚创建`java-fix-npe-in-integer-comparison`的项目文件夹下的`pom.xml`并打开。
