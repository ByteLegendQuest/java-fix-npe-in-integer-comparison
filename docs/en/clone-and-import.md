...
`git clone https://github.com/ByteLegendQuest/java-fix-npe-in-integer-comparison`

A project directory named `java-fix-npe-in-integer-comparison` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-npe-in-integer-comparison` directory which is created just now.
